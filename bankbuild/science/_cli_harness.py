...
USAGE (from the CLI orchestrator):
  python -m bankbuild.science._cli_harness prompt_research     <idx>
  python -m bankbuild.science._cli_harness prompt_author       <idx>
  python -m bankbuild.science._cli_harness prompt_craft_judge  <idx>
  python -m bankbuild.science._cli_harness prompt_adv_judge    <idx>
  python -m bankbuild.science._cli_harness prompt_revise       <idx>  (auto-detects which stage)
  python -m bankbuild.science._cli_harness prompt_coordinator  <idx1> [<idx2> ...]  (batch research+author+self-audit prompt)
  python -m bankbuild.science._cli_harness mechgate            <idx>  (writes gate_flags.json)
  python -m bankbuild.science._cli_harness apply_drops         <idx1> [<idx2> ...]  (drop still-flagged rungs from latest ladder + verdicts)
  python -m bankbuild.science._cli_harness state               <idx>  (dump what stages have run)
  python -m bankbuild.science._cli_harness finalize <idx1> [<idx2> ...]  (writes wf-style output)
"""
...
SUBJECT = "science"
CFG_PATH   = os.path.join(ROOT, "bankbuild", "subjects", "science.json")
QUEUE_PATH = os.path.join(ROOT, "bankbuild", "science", "_queue.json")
STATE_DIR  = os.path.join(ROOT, "bankbuild", "science", "_cli_state")
...
{len(idxs)} science topics.  You do this SEQUENTIALLY in your own context (do NOT try to spawn
...
def finalize(idxs):
    ...
    # Auto-integrate against the science subject so we never accidentally point
    # bank.py integrate at the wrong queue (idx collides across subjects).
    import subprocess
    print("--- auto-integrating against science ---")
