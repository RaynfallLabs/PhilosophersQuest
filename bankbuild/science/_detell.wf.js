export const meta = {
  name: 'science-detell',
  description: 'Surgical de-tell of needs_review SCIENCE ladders (config-driven): reviser fixes/drops ONLY flagged rungs (rest byte-identical, NO new facts), then a fresh ADVERSARIAL judge + deterministic mechanical gate confirm 0 high + 0 medium. Knowledge-subject standard: the player must KNOW the science fact, not deduce it from the stem.',
  phases: [
...
// args.ids = needs_review ladder ids to de-tell. args.config optional (defaults to the embedded science config).
const A = (typeof args === 'string') ? JSON.parse(args) : (args || {});
const CONFIG = {
  "name": "science",
  "tier_note": "Tiers = CONCEPTUAL difficulty (T1 = concrete recognition a kid can picture .. T5 = analytic mechanism / contested-topic reasoning), grade-10 HARD ceiling. Operative gameplay gate = TOTAL record chars (stem + 4 choices): T1<=460, T2<=580, T3<=720, T4<=850, T5<=1050; context is UNCAPPED. Science is CHAIN mode (~36s at WIS 10); each rung a DIFFERENT wonder of the topic.",
  "voice_rule": "SCIENCE = the DISCOVERY PATTERN: the most memorable question reveals HOW WE KNOW what we know -- and leaves the kid wanting to TEST the world personally (skepticism of your OWN claims first; Feynman's 'you are the easiest person to fool'). Four modes: DISCOVERY (named person + specific moment/experiment), REVERSAL (consensus wrong, the TEST decided), MYSTERY ('no one yet knows', said proudly -- never let a distractor secretly be the truth), MECHANISM (counter-intuitive how-it-works a paragraph conveys). ANSWER HIERARCHY: NAMED people/moments/experiments and VIVID mechanisms beat everything; a bare generic label ('an experiment', 'a chemical reaction', 'scientists'), bare date, or rote symbol/definition is BANNED as an answer. Dinner Test: would a kid excitedly retell THIS answer? Scene-led, never 'What is photosynthesis?' / 'Who discovered X?'. ONE COHERENT WONDER PER RUNG: hook + setup + answer all the SAME fact. DISTRACTOR & ANTI-RESTATEMENT DISCIPLINE (critical): the stem sets up the PUZZLE -- NEVER the SOLUTION the answer names. If the stem describes the mechanism/outcome and the answer merely names it, that is RESTATEMENT and the player DEDUCES instead of KNOWS. FIX: describe the scene/challenge (what Semmelweis SAW: two wards, tripled deaths) and let the ANSWER supply the fact as NEW information. ALL FOUR choices must stay LIVE -- never let a stem clause self-eliminate a distractor; distractors are adjacent-but-wrong REAL science (the respectable-but-wrong graveyard: phlogiston, caloric, miasma, stress-ulcers, steady state, the ether -- plus real adjacent scientists / rival mechanisms). Never DEFINE a term then ask its label. CHOICE PARITY: all four structurally parallel -- if the answer is a named person, all four are named people. Numbers are answers ONLY when the stem builds a specific expectation the number SHATTERS. Myths stay flagged AS myths (the apple, the Pisa drop, Kekule's dream); corrected-misconception rungs keep their correction intact.",
  "framing": "The bank's soul is METHOD vs INSTITUTION: science-the-method celebrated, science-the-institution covered honestly (capture, censorship, the dissent tradition Galileo -> Semmelweis -> Wegener -> Marshall -> GBD -> Bhattacharya). Stance topics LEAN where the config commits (climate per Koonin, COVID honest, vaccines SCRUTINIZED not celebrated, eugenics/Lysenko full depth, WEF named) -- PRESENT, never preach; steel-man, never sneer. The de-tell must never soften or reverse a keyed stance fact, and never harden a neutral rung into advocacy. The answer is always the science fact/moment/mechanism, never an adjacent war/politics episode."
};
const CFG = A.config || CONFIG;
const IDS = Array.isArray(A.ids) ? A.ids : [];
const SUBJECT  = CFG.name || 'science';
...
1. LEAD WITH THE SUBJECT (no dangling pronoun before the named person/experiment; parse in ONE forward pass).
...
8. SCENE-SETTING MUST ORIENT (which discovery/experiment this is, what's remarkable, why it matters) -- keep the vivid scene, it is the wonder.
10. THE ANSWER MUST ITSELF BE THE PAYOFF the voice prizes (the named person/moment/experiment, the vivid mechanism), never a dead label/bare superlative/bare date/generic phrase.
...
FACTUAL INTEGRITY: do NOT introduce, change, or invent any factual claim. The keyed science facts were already researched, sourced, and judged correct -- preserve every keyed FACT exactly (including myth-flags and corrected-misconception framings). You are removing TELEGRAPHS, not re-researching.`;
...
function reviserPrompt(id, priorFlags, isFinal){ return `You are the SURGICAL DE-TELL reviser for a needs_review ${SUBJECT} ladder. A strict adversarial judge flagged a few rungs for TELEGRAPHS -- the answer can be picked WITHOUT knowing the science fact. Remove those telegraphs while PRESERVING the vivid content.
...
WHAT IS / ISN'T A TELL (this is a KNOWLEDGE subject -- the player must KNOW the science fact, not DEDUCE it from the stem):
- A real TELL is when the answer is pickable WITHOUT knowing the fact: (a) RESTATEMENT -- the stem describes the mechanism/outcome and the answer merely names it; (b) DEFINE-THEN-LABEL -- the stem fully describes a thing then asks only for its name/term; (c) ODD-ONE-OUT -- the answer is the structural standout (the only proper name, number, long one, different shape); (d) a STEM WORD hands over the answer; (e) the distractors are OFF-TOPIC / factually dead / SELF-ELIMINATE against a stem premise, or the stem ENUMERATES items matching the distractors so the answer is the only un-listed one.
- It is GOOD to set a vivid scene -- do NOT strip the wonder. The fix is to move the giveaway OUT of the stem so the ANSWER carries the science fact as NEW information, and to make all four distractors LIVE.

HOW TO FIX (fix > drop; reconceive, don't patch):
- RESTATEMENT / DEFINE-THEN-LABEL: reword the STEM so it sets up the scene/challenge WITHOUT stating the mechanism or naming the term; let the ANSWER supply the fact as new information. If a rung is fundamentally 'describe X then name X', reconceive it to test a fact the player must actually KNOW.
- PARITY (sacred): after editing, ALL FOUR choices share the same opening, grammatical shape, and similar length -- the answer must NEVER be the odd-one-out. If the answer is a named person, make all four named people; if a mechanism phrase, all four mechanism phrases. Reword the others to match.
- DEAD / SELF-ELIMINATING DISTRACTORS: replace them with LIVE, plausible wrong science a kid could actually believe (real rival theories, real adjacent scientists/mechanisms) -- never one the stem's own words rule out. The kid must KNOW the right one, not eliminate the impossible.
- Reword the STEM only enough to stop it stating/echoing the answer; keep the vivid scene and the named anchor.
- NEVER introduce, change, or invent a FACT. The keyed fact is already sourced -- preserve it exactly (myth-flags and misconception-corrections included).
...
${RULES}
THE STANDARD (this is a KNOWLEDGE subject): the player must KNOW the science fact -- flag a rung when the answer can be picked WITHOUT knowing it:
(a) the answer RESTATES a clause the stem gave, or the stem DEFINES-THEN-asks-the-LABEL; (b) the answer is the structural ODD-ONE-OUT (shape / length / lone proper-name / lone number); (c) a STEM WORD hands over the answer; (d) the three distractors are off-topic / factually dead / self-eliminate against a stem premise, or the stem ENUMERATES the distractors, so the answer survives by elimination. Do NOT flag a rung merely for having a vivid scene when the answer is a genuine science fact the player must KNOW and the three distractors are LIVE, plausible wrong science.
