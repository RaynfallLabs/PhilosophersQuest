"""Assemble the per-slice spec files into register.json + _queue.json for the science bank v2.

Reads bankbuild/science/_strands/spec_*.json (each a JSON array of topic-ladder specs), adds a unique
...
deep>standard>mini), groups by `section` (physics|chemistry|life|earthspace|howscience), and writes
both files with coverage stats. Re-runnable.

Usage: python bankbuild/science/_assemble_queue.py
"""
...
SD    = os.path.join(ROOT, "bankbuild", "science", "_strands")
...
SORDER = {"physics": 0, "chemistry": 1, "life": 2, "earthspace": 3, "howscience": 4}
SECTION_NAME = {
    "physics":    "Physics (motion, energy, light, quantum, atoms -- told by the experiments that showed it)",
    "chemistry":  "Chemistry (element hunters, bonds, reactions, materials & the prepared mind)",
    "life":       "Life & medicine (cells, DNA, evolution, the body, germ theory, vaccines-scrutinized)",
    "earthspace": "Earth & space (deep time, honest climate, the mapped heavens; mechanism not place)",
    "howscience": "How science works (method, dissent, frauds, dark chapters, contested topics -- heaviest)",
}
...
reg = {"subject": "science", "total_topics": len(topics),
