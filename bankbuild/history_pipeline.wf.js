...
3. NO LEXICAL/CATEGORY TELEGRAPH. No stem word that hands over the answer: not the answer's key noun, not a category word only the answer matches ("tree" when only the answer is a tree; a setup "about her body" when only the wound answer is bodily), not a verb revealing the mechanism ("sent an armorer to DIG for it" -> "buried"), not a tone-word matching only the answer. The player must KNOW the answer, not deduce it from a stem word.
...
13. NO LOGICAL TELEGRAPH. Distractors must not be self-eliminating against a stem premise ("Historians say it never happened" + three distractors that assert it DID happen forces the answer). Every distractor must be a live, plausible option given everything the stem says.
