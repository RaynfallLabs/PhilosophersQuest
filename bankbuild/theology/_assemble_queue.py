"""Assemble the per-slice spec files into register.json + _queue.json for the theology bank v2.

Reads bankbuild/theology/_strands/spec_*.json (each a JSON array of topic-ladder specs), adds a unique
...
deep>standard>mini), groups by `section` (christian|greek|norse|western_myth), and writes
both files with coverage stats. Re-runnable.

Usage: python bankbuild/theology/_assemble_queue.py
"""
...
SD    = os.path.join(ROOT, "bankbuild", "theology", "_strands")
QUEUE = os.path.join(ROOT, "bankbuild", "theology", "_queue.json")
REG   = os.path.join(ROOT, "bankbuild", "theology", "register.json")
...
SORDER = {"christian": 0, "greek": 1, "norse": 2, "western_myth": 3}
SECTION_NAME = {
    "christian":    "Christian (OT stories, NT stories, apostles + early church, saint-legends)",
    "greek":        "Greek (Olympians + Titans, Trojan War + Odyssey, heroes and their labors)",
    "norse":        "Norse (Aesir gods, Loki's tricks + the six treasures, Ragnarok + Volsung + Beowulf)",
    "western_myth": "Western myth (Arthurian, Robin Hood/Roland/El Cid, Tuatha De Danann, Cu Chulainn + Fianna)",
}
...
reg = {"subject": "theology", "total_topics": len(topics),
