export const meta = {
  name: 'animal-detell',
  description: 'Surgical de-tell of needs_review ANIMAL ladders (config-driven): reviser fixes/drops ONLY flagged rungs (rest byte-identical, NO new facts), then a fresh ADVERSARIAL judge + deterministic mechanical gate confirm 0 high + 0 medium. Knowledge-subject standard: the player must KNOW the animal fact, not deduce it.',
  phases: [
...
// args.ids = needs_review ladder ids to de-tell. args.config optional (defaults to the embedded animal config).
const A = (typeof args === 'string') ? JSON.parse(args) : (args || {});
const CONFIG = {
  "name": "animal",
  "tier_note": "Tiers = CONCEPTUAL difficulty (T1 = recognition a kid can almost eyeball .. T5 = deep paleobiology / contested-debate framing), grade-10 ceiling. Operative gameplay gate = TOTAL record chars (stem + 4 choices): T1<=500, T2<=620, T3<=770, T4<=900, T5<=1100; context is UNCAPPED. Animal is CHAIN mode; each rung a DIFFERENT wonder.",
  "voice_rule": "ANIMAL = pure animal-KNOWLEDGE: amazing biology, behavior, senses, adaptation, life cycle, paleobiology, and how creatures SURVIVE -- plus the truly BIZARRE animals. THE ANIMAL IS ALWAYS THE ANCHOR: every rung's ANSWER is a fact about an animal. ONE COHERENT WONDER PER RUNG: hook + setup + answer all about the SAME fact; never staple an unrelated detail onto a different answer. WONDER PATTERN answer hierarchy: NAMED THINGS > VIVID ACTIONS/ADAPTATIONS > CONCRETE OBJECTS > NUMBERS (only when the stem CONSTRUCTS the number as a payoff; a cold number = BANNED) > GENERIC LABELS (banned as answers). Dinner Test: would a kid excitedly retell THIS answer at dinner? Scene-led, never 'What is X?'. Concrete handles beat jargon. DISTRACTOR & ANTI-RESTATEMENT DISCIPLINE (critical): the stem sets up the CHALLENGE/SCENE/PUZZLE the animal faces -- NEVER the SOLUTION the answer names. If the stem describes the mechanism/trait/effect and the answer merely names it, that is RESTATEMENT and the player DEDUCES instead of KNOWS. FIX: describe the situation and let the ANSWER supply the mechanism as NEW information. ALL FOUR choices must stay LIVE given everything the stem says -- never let a stem clause self-eliminate a distractor. Never DEFINE a term then ask for its label. CHOICE PARITY: all four choices structurally parallel -- if the answer is a proper name, all four are proper names. Numbers are answers ONLY when the stem builds a specific expectation the number SHATTERS.",
  "framing": "Mostly SETTLED science, presented with WONDER, not advocacy. Evolution + deep timeline = standard science. Reproductive/sexual diversity in OTHER species is wonder on its OWN terms, NEVER a human-sex analogy. ONE contested topic (both sides, no verdict): Pleistocene overkill (Martin 1967) vs. climate. No anthropomorphizing as fact; flag research-in-progress honestly. The answer is always the ANIMAL, never an adjacent human layer."
};
const CFG = A.config || CONFIG;
const IDS = Array.isArray(A.ids) ? A.ids : [];
const SUBJECT  = CFG.name || 'animal';
...
8. SCENE-SETTING MUST ORIENT (who/what the animal is, what's at stake) -- keep the vivid scene, it is the wonder.
10. THE ANSWER MUST ITSELF BE THE PAYOFF the voice prizes (the memorable animal fact), never a dead label/jargon term or a bare number.
13. NO LOGICAL TELEGRAPH. Every distractor must be a LIVE, plausible option given everything the stem says. Kill: SELF-ELIMINATION (a stem premise rules a distractor dead -- 'no skeleton' kills 'shoulder bone'); ENUMERATION (the stem lists items matching the distractors, leaving the answer the only un-listed one); CATEGORY-MATCH (only the answer fits a category the stem names); EFFECT/GOAL-MATCH (the stem states the outcome only the answer achieves).

VALUES: ${FRAMING}

FACTUAL INTEGRITY: do NOT introduce, change, or invent any factual claim. The keyed animal facts were already researched, sourced, and judged correct -- preserve every keyed FACT exactly. You are removing TELEGRAPHS, not re-researching.`;
...
function reviserPrompt(id, priorFlags, isFinal){ return `You are the SURGICAL DE-TELL reviser for a needs_review ${SUBJECT} ladder. A strict adversarial judge flagged a few rungs for TELEGRAPHS -- the answer can be picked WITHOUT knowing the animal fact. Remove those telegraphs while PRESERVING the vivid content.
...
WHAT IS / ISN'T A TELL (this is a KNOWLEDGE subject -- the player must KNOW the animal fact, not DEDUCE it from the stem):
- A real TELL is when the answer is pickable WITHOUT knowing the animal fact: (a) RESTATEMENT -- the stem describes the mechanism/trait/effect and the answer merely names it; (b) DEFINE-THEN-LABEL -- the stem fully describes a thing then asks only for its name/term; (c) ODD-ONE-OUT -- the answer is the structural standout (the only proper name, number, long one, different shape); (d) a STEM WORD hands over the answer; (e) the distractors are OFF-TOPIC / factually dead / SELF-ELIMINATE against a stem premise, or the stem ENUMERATES items matching the distractors so the answer is the only un-listed one.
- It is GOOD to set a vivid scene -- do NOT strip the wonder. The fix is to move the giveaway OUT of the stem so the ANSWER carries the animal fact as NEW information, and to make all four distractors LIVE.

HOW TO FIX (fix > drop; reconceive, don't patch):
- RESTATEMENT / DEFINE-THEN-LABEL: reword the STEM so it sets up the challenge/scene/predator WITHOUT stating the mechanism or naming the term; let the ANSWER supply the animal fact as new information. If a rung is fundamentally 'describe X then name X', reconceive it to test a fact the player must actually KNOW.
...
- DEAD / SELF-ELIMINATING DISTRACTORS: replace them with LIVE, plausible wrong animal facts a kid could actually believe -- never one the stem's own words rule out. The kid must KNOW the right one, not eliminate the impossible.
- Reword the STEM only enough to stop it stating/echoing the answer; keep the vivid scene.
- NEVER introduce, change, or invent a FACT. The keyed animal fact is already sourced -- preserve it exactly.
...
${RULES}
THE STANDARD (this is a KNOWLEDGE subject): the player must KNOW the animal fact -- flag a rung when the answer can be picked WITHOUT knowing it:
(a) the answer RESTATES a clause the stem gave, or the stem DEFINES-THEN-asks-the-LABEL; (b) the answer is the structural ODD-ONE-OUT (shape / length / lone proper-name / lone number); (c) a STEM WORD hands over the answer; (d) the three distractors are off-topic / factually dead / self-eliminate against a stem premise, or the stem ENUMERATES the distractors, so the answer survives by elimination. Do NOT flag a rung merely for having a vivid scene when the answer is a genuine animal fact the player must KNOW and the three distractors are LIVE, plausible wrong facts.
