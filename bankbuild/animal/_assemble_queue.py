"""Assemble the per-slice spec files into register.json + _queue.json for the animal bank v2.

Reads bankbuild/animal/_strands/spec_*.json (each a JSON array of topic-ladder specs), adds a unique
...
deep>standard>mini), groups by `section` (adaptation|creature|bizarre|paleo|contested), and writes both
files with coverage stats. Re-runnable.

Usage: python bankbuild/animal/_assemble_queue.py
"""
import json, os, re, collections

ROOT = r"C:\Users\brand\Documents\PhilosophersQuest"
SD    = os.path.join(ROOT, "bankbuild", "animal", "_strands")
QUEUE = os.path.join(ROOT, "bankbuild", "animal", "_queue.json")
REG   = os.path.join(ROOT, "bankbuild", "animal", "register.json")
...
SECTION_NAME = {
    "adaptation": "Adaptation ladders (theme deepened across animals)",
...
STOP = set("the a an of and or to in on at by for is as vs its s".split())
...
reg = {"subject": "animal", "total_topics": len(topics),
