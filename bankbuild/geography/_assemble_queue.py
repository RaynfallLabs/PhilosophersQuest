"""Assemble the per-slice spec files into register.json + _queue.json for the geography bank v2.

Reads bankbuild/geography/_strands/spec_*.json (each a JSON array of topic-ladder specs), adds a unique
...
deep>standard>mini), groups by `section` (city|nature|ancient|culture|earthsystem), and writes
both files with coverage stats. Re-runnable.

Usage: python bankbuild/geography/_assemble_queue.py
"""
...
QUEUE = os.path.join(ROOT, "bankbuild", "geography", "_queue.json")
REG   = os.path.join(ROOT, "bankbuild", "geography", "register.json")
...
DORDER = {"deep": 0, "standard": 1, "mini": 2}
...
SECTION_NAME = {
    ...
    "ancient":     "Ancient marvels & lost civilizations (place-wonder; flag history-bank overlap)",
    "culture":     "Cultural birthplaces (a dance/craft/script born in a place; NON-food)",
    "earthsystem": "Magical earth systems (one process deepened ACROSS the places that show it)",
}
...
# priority sort for build order: weight, then depth, then section, strand, name
...
# register grouped by section -> strand
...
reg = {"subject": "geography", "total_topics": len(topics),
...
print(f"TOPICS: {len(topics)}   SUM target_q (est questions): {reg['sum_target_q']}   "
      f"vision_mandated: {reg['vision_mandated']}   history_overlap flagged: {reg['history_overlap_flagged']}")
...
for k in sorted(sec, key=lambda x: SORDER.get(x, 9)):
    c, qq = sec[k]
    print(f"   {c:3d} top / {qq:4d} q ({100*qq//tot_q:2d}%)  {k:11s} {SECTION_NAME.get(k,'')}")
print(f"\nHISTORY-OVERLAP flagged topics (review before build): {reg['history_overlap_flagged']}")
for r in topics:
