export const meta = {
  name: 'cooking-detell',
  description: 'Surgical de-tell of needs_review COOKING ladders (config-driven): reviser fixes/drops ONLY flagged rungs (rest byte-identical, NO new facts), then a fresh ADVERSARIAL judge + deterministic mechanical gate confirm 0 high + 0 medium. Knowledge-subject standard: the player must KNOW the food/cooking fact, not deduce it from the stem.',
  phases: [
...
// args.ids = needs_review ladder ids to de-tell. args.config optional (defaults to the embedded cooking config).
const A = (typeof args === 'string') ? JSON.parse(args) : (args || {});
const CONFIG = {
  "name": "cooking",
  "tier_note": "Tiers = CONCEPTUAL / GRADE difficulty (T1 = a 10-year-old can know/reason it .. T5 = a food-science concept a bright 15-year-old can reason), grade-10 ceiling. Operative gameplay gate = TOTAL record chars (stem + 4 choices): T1<=500, T2<=620, T3<=770, T4<=900, T5<=1100; context is UNCAPPED. Cooking is ESCALATOR_CHAIN mode; each rung a DIFFERENT payoff. BANNED MINUTIAE as the tested ANSWER: exact numbers/yields/percentages, microbe/enzyme Latin names, lab jargon, production statistics -- teach the CONCEPT (a number/microbe may be a distractor or context only).",
  "voice_rule": "COOKING = food KNOWLEDGE + WONDER a kid can USE: how cooking works (mechanism), where food comes from, what a dish IS, food history's drama, amazing food facts, and what food does in the body. THE ANSWER IS ALWAYS FOOD-ANCHORED: every rung's ANSWER is a fact about food, cooking, an ingredient, a technique, a dish, or a food-driven payoff -- NEVER a bare law/policy/agency or a human name/date with no food in it. ONE COHERENT POINT PER RUNG: hook + setup + answer all about the SAME fact; never staple an unrelated detail onto a different answer. ANSWER HIERARCHY: NAMED THINGS > VIVID ACTIONS/MECHANISMS > CONCRETE OBJECTS/PARTS > NUMBERS (only when the stem CONSTRUCTS the number as a payoff; a cold number = BANNED) > GENERIC LABELS (banned as answers). Dinner Test: would a kid excitedly retell THIS answer? Scene-led, never 'What is X?' / 'Define Y'. Concrete handles beat jargon. DISTRACTOR & ANTI-RESTATEMENT DISCIPLINE (critical -- KNOWLEDGE subject, deducing the answer from the stem IS the tell): the stem sets up the DISH/SCENE/PUZZLE/CHALLENGE -- NEVER the SOLUTION the answer names. If the stem describes the technique/effect and the answer merely names it, that is RESTATEMENT and the player DEDUCES instead of KNOWS. FIX: describe the situation and let the ANSWER supply the fact/mechanism/name/part as NEW information. ALL FOUR choices must stay LIVE given everything the stem says -- never let a stem clause self-eliminate a distractor. Never DEFINE a term then ask for its label. CHOICE PARITY: all four choices structurally parallel -- if the answer is a proper name, all four are proper names. Numbers are answers ONLY when the stem builds a specific expectation the number SHATTERS. Prep-frame ('you sear/knead/cut it') is FINE and kid-appropriate.",
  "framing": "Mostly PRACTICAL skill + SETTLED food science + honest, dramatic food history, presented with WONDER and USEFULNESS, never advocacy or moralizing (no food-shaming). Nutrition LEANS TRADITIONAL-FOODS: steel-man both sides, lean skeptical of ultra-processed / heated-seed-oil orthodoxy, sympathetic to whole/traditional/fermented foods, empowerment not preaching. FIRM where evidence is strong (danger zone, cross-contamination, trans fats, reused-fryer-oil aldehydes, nixtamalization prevents pellagra). ADJACENCY GUARD: the answer is always the FOOD -- if the real substance is an adjacent human-policy layer (a law, an agency, food-aid geopolitics), it belongs in history/economics."
};
const CFG = A.config || CONFIG;
const IDS = Array.isArray(A.ids) ? A.ids : [];
const SUBJECT  = CFG.name || 'cooking';
...
8. SCENE-SETTING MUST ORIENT (who/what the dish/ingredient is, what's at stake) -- keep the vivid scene, it is the wonder.
10. THE ANSWER MUST ITSELF BE THE PAYOFF the voice prizes (the memorable food fact), never a dead label/jargon term or a bare number.
...
FACTUAL INTEGRITY: do NOT introduce, change, or invent any factual claim. The keyed food facts were already researched, sourced, and judged correct -- preserve every keyed FACT exactly. You are removing TELEGRAPHS, not re-researching.`;
...
function reviserPrompt(id, priorFlags, isFinal){ return `You are the SURGICAL DE-TELL reviser for a needs_review ${SUBJECT} ladder. A strict adversarial judge flagged a few rungs for TELEGRAPHS -- the answer can be picked WITHOUT knowing the food/cooking fact. Remove those telegraphs while PRESERVING the vivid content.
...
WHAT IS / ISN'T A TELL (this is a KNOWLEDGE subject -- the player must KNOW the food/cooking fact, not DEDUCE it from the stem):
- A real TELL is when the answer is pickable WITHOUT knowing the fact: (a) RESTATEMENT -- the stem describes the technique/mechanism/effect and the answer merely names it; (b) DEFINE-THEN-LABEL -- the stem fully describes a thing then asks only for its name/term; (c) ODD-ONE-OUT -- the answer is the structural standout (the only proper name, number, long one, different shape); (d) a STEM WORD hands over the answer; (e) the distractors are OFF-TOPIC / factually dead / SELF-ELIMINATE against a stem premise, or the stem ENUMERATES items matching the distractors so the answer is the only un-listed one.
- It is GOOD to set a vivid scene -- do NOT strip the wonder. The fix is to move the giveaway OUT of the stem so the ANSWER carries the food fact as NEW information, and to make all four distractors LIVE.

HOW TO FIX (fix > drop; reconceive, don't patch):
- RESTATEMENT / DEFINE-THEN-LABEL: reword the STEM so it sets up the dish/scene/challenge WITHOUT stating the mechanism or naming the term; let the ANSWER supply the food fact as new information. If a rung is fundamentally 'describe X then name X', reconceive it to test a fact the player must actually KNOW.
- PARITY (sacred): after editing, ALL FOUR choices share the same opening, grammatical shape, and similar length -- the answer must NEVER be the odd-one-out. If the answer is a proper name, make all four proper names; if a phrase, all four phrases. Reword the others to match.
- DEAD / SELF-ELIMINATING DISTRACTORS: replace them with LIVE, plausible wrong food facts a kid could actually believe -- never one the stem's own words rule out. The kid must KNOW the right one, not eliminate the impossible.
- Reword the STEM only enough to stop it stating/echoing the answer; keep the vivid scene.
- NEVER introduce, change, or invent a FACT. The keyed food fact is already sourced -- preserve it exactly.
...
${RULES}
THE STANDARD (this is a KNOWLEDGE subject): the player must KNOW the food/cooking fact -- flag a rung when the answer can be picked WITHOUT knowing it:
(a) the answer RESTATES a clause the stem gave, or the stem DEFINES-THEN-asks-the-LABEL; (b) the answer is the structural ODD-ONE-OUT (shape / length / lone proper-name / lone number); (c) a STEM WORD hands over the answer; (d) the three distractors are off-topic / factually dead / self-eliminate against a stem premise, or the stem ENUMERATES the distractors, so the answer survives by elimination. Do NOT flag a rung merely for having a vivid scene when the answer is a genuine food fact the player must KNOW and the three distractors are LIVE, plausible wrong facts.
