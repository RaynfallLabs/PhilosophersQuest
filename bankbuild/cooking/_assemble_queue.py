"""Assemble the per-slice spec files into register.json + _queue.json for the cooking bank v2.

Reads bankbuild/cooking/_strands/spec_*.json (each a JSON array of topic-ladder specs), adds a unique
...
deep>standard>mini), groups by `section` (kitchen|ingredient|recipe|history|wonder|nutrition), and writes
both files with coverage stats. Re-runnable.

Usage: python bankbuild/cooking/_assemble_queue.py
"""
import json, os, re, collections

ROOT = r"C:\Users\brand\Documents\PhilosophersQuest"
SD    = os.path.join(ROOT, "bankbuild", "cooking", "_strands")
QUEUE = os.path.join(ROOT, "bankbuild", "cooking", "_queue.json")
REG   = os.path.join(ROOT, "bankbuild", "cooking", "register.json")
...
SECTION_NAME = {
    ...
    "ingredient": "Ingredients (provenance -- where food comes from, cuts, parts of the plant)",
    "recipe":     "Recipes (dish literacy -- what a classic dish is + its core technique)",
    "history":    "Food history (world history through food, drama-led)",
...
for fn in spec_files:
    ...
    for t in arr:
        ...
        rec = {"id": sid, "name": t["name"], "section": t["section"], "strand": t["strand"],
               ...
               "vision_mandated": bool(t["vision_mandated"]), "weight": t["weight"],
               "depth": t["depth"], "target_q": int(t["target_q"])}
        topics.append(rec)
...
reg = {"subject": "cooking", "total_topics": len(topics),
...
print(f"TOPICS: {len(topics)}   SUM target_q (est questions): {reg['sum_target_q']}   "
      f"vision_mandated: {reg['vision_mandated']}")
