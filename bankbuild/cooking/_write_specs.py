"""Read the queue-research workflow output, write one spec_<slug>.json per sub-strand.

Injects the fixed section/strand/kind fields (the agents omitted them) onto each returned spec.
Usage: python bankbuild/cooking/_write_specs.py "<task output .output file>"
"""
import json, os, sys
...
SPEC_FIELDS = ["name", "scope", "tier_span", "depth", "target_q", "source",
               "framing_note", "vision_mandated", "weight"]
total_specs = 0
rows = []
for ss in subs:
...
for sec in ("kitchen", "ingredient", "recipe", "history", "wonder", "nutrition"):
    ...
    if secrows:
        ...
        for _, slug, n, q in secrows:
            print(f"    {slug:32s} {n:3d} topics  ~{q:3d} q")
