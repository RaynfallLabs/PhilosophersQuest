...
USAGE (from the CLI orchestrator):
  python -m bankbuild.grammar._cli_harness prompt_research     <idx>
  python -m bankbuild.grammar._cli_harness prompt_author       <idx>
  python -m bankbuild.grammar._cli_harness prompt_craft_judge  <idx>
  python -m bankbuild.grammar._cli_harness prompt_adv_judge    <idx>
  python -m bankbuild.grammar._cli_harness prompt_revise       <idx>  (auto-detects which stage)
  python -m bankbuild.grammar._cli_harness prompt_coordinator  <idx1> [<idx2> ...]  (batch research+author+self-audit prompt)
  python -m bankbuild.grammar._cli_harness mechgate            <idx>  (writes gate_flags.json)
  python -m bankbuild.grammar._cli_harness apply_drops         <idx1> [<idx2> ...]  (drop still-flagged rungs from latest ladder + verdicts)
  python -m bankbuild.grammar._cli_harness state               <idx>  (dump what stages have run)
  python -m bankbuild.grammar._cli_harness finalize <idx1> [<idx2> ...]  (writes wf-style output)
"""
...
SUBJECT = "grammar"
CFG_PATH   = os.path.join(ROOT, "bankbuild", "subjects", "grammar.json")
QUEUE_PATH = os.path.join(ROOT, "bankbuild", "grammar", "_queue.json")
...
=== GRAMMAR CALIBRATION (READ BEFORE FLAGGING) ===
This is a SNAPPY-ROTE-WITH-A-VOICE bank, NOT a wonder-bank and NOT a reasoning-bank. The Comma-Saves-Lives voice EXPECTS quoted-misuse punchlines + short paradigm forms + category names (at T1-T3) as the memorable payoff. Adjust these instincts before flagging:

1. PUNCHLINE-VIA-MISUSE IS THE VOICE, not "restatement". The controlling pattern quotes the WRONG version of a sentence in the STEM and asks what was broken -- e.g. stem quotes "Let's eat Grandma!" and answer is "missing vocative comma". The stem and answer BOTH point at the same construction ON PURPOSE; that is CORRECT teaching, not restatement. Flagging the Comma-Saves-Lives shape as restatement is a FALSE POSITIVE.

2. GRAMMAR CATEGORY NAMES ARE LEGITIMATE PAYOFFS at T1-T3. Answers like "vocative comma", "comma splice", "subject-verb agreement", "dangling modifier", "misplaced modifier", "run-on sentence", "sentence fragment", "possessive apostrophe", "chiasmus", "zeugma", "malapropism", "spoonerism", "garden-path sentence", "parallel structure" are ON-VOICE for a snappy-rote grammar bank -- do NOT flag them as "dead names" or "generic labels". The bank is TEACHING these terms; T4-T5 shifts to subtler judgments where the term becomes distractor rather than answer.

3. QUOTED WRONG SENTENCES IN THE STEM ARE THE SCENE. A stem that reads 'Which mistake breaks "I invited the strippers, JFK and Stalin"?' has the punchline quoted -- the quote IS the scene. Do NOT flag as "stem gives away the answer" merely because the wrong-sentence and the rule-name obviously point at the same construction; that IS the pedagogical shape.

4. PARADIGM/INFLECTION ANSWERS ARE LEGITIMATE. "who / whom", "lay / lie", "had run (not had ran)", "fewer (not less)", "affect (verb) / effect (noun)" are on-voice. Do NOT flag as "definition-drill" -- these are RECOGNITION-of-the-correct-form, which is the T2-T3 pattern.

5. CATEGORY-MATCH is expected in category-naming ladders BY DESIGN. When a stem quotes a wrong sentence and asks which grammar rule was broken, all four choices SHOULD be plausible grammar rules. That is PARITY, not category-match failure. Only flag category-match when ONE choice fits a stated category and the OTHER THREE OBVIOUSLY DO NOT.

STILL FLAG when genuine problems exist: fabricated etymology facts, wrong attributions ("Lynne Truss wrote 'Eats, Shoots and Leaves' in 2005" -- it's 2003), politically-tinted example sentences dragging ideology into grammar, above-grade-10 linguistics jargon (Chomsky, Saussure, transformational, morphosyntax) as ANSWERS, and stems that literally spell the answer term BEFORE quoting the wrong sentence (the wrong-sentence quote is the scene, the term is the payoff -- reversed order leaks).

For EACH rung: verdict 'keep' or 'flag'; if flag: severity (high/medium/low) + rules_flagged + primary_flaw + fix. Be the adversary the first judge wasn't -- BUT do not create false positives against the Comma-Saves-Lives voice. ladder_ok = true ONLY if nothing is HIGH or MEDIUM.
...
def finalize(idxs):
    ...
    # Auto-integrate against the grammar subject so we never accidentally point
