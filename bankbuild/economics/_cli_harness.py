...
USAGE (from the CLI orchestrator):
  python -m bankbuild.economics._cli_harness prompt_research     <idx>
  python -m bankbuild.economics._cli_harness prompt_author       <idx>
  python -m bankbuild.economics._cli_harness prompt_craft_judge  <idx>
  python -m bankbuild.economics._cli_harness prompt_adv_judge    <idx>
  python -m bankbuild.economics._cli_harness prompt_revise       <idx>  (auto-detects which stage)
  python -m bankbuild.economics._cli_harness prompt_coordinator  <idx1> [<idx2> ...]  (batch research+author+self-audit prompt)
  python -m bankbuild.economics._cli_harness mechgate            <idx>  (writes gate_flags.json)
  python -m bankbuild.economics._cli_harness apply_drops         <idx1> [<idx2> ...]  (drop still-flagged rungs from latest ladder + verdicts)
  python -m bankbuild.economics._cli_harness state               <idx>  (dump what stages have run)
  python -m bankbuild.economics._cli_harness finalize <idx1> [<idx2> ...]  (writes wf-style output)
"""
...
SUBJECT = "economics"
CFG_PATH   = os.path.join(ROOT, "bankbuild", "subjects", "economics.json")
QUEUE_PATH = os.path.join(ROOT, "bankbuild", "economics", "_queue.json")
STATE_DIR  = os.path.join(ROOT, "bankbuild", "economics", "_cli_state")
...
=== ECONOMICS CALIBRATION (READ BEFORE FLAGGING) ===
This is a REASONING-bank, NOT a wonder-bank. The Bastiat Pattern voice EXPECTS named reasoning-moves + named economists + specific historical events as the memorable payoff. Adjust these instincts before flagging:

1. NAMED REASONING-MOVES ARE LEGITIMATE PAYOFFS, not "generic labels" or "dead names". These are all on-voice ANSWERS and MUST NOT be flagged as Drama-Available or generic-label:
   - "the Cantillon effect", "the calculation problem", "the knowledge problem", "the seen and the unseen", "spontaneous order", "moral hazard", "rent-seeking", "regulatory capture", "public choice", "constructivist rationalism", "praxeology", "the natural rate hypothesis", "opportunity cost", "time preference", "roundabout production", "Bootleggers and Baptists"
   - Named critic moves: "compared to what?", "judge by results not intentions", "there are no solutions only trade-offs", "inflation is always and everywhere a monetary phenomenon"
   - These are DESIGNED as the memorable answers per the controlling voice (Bastiat Pattern). Flagging them is a FALSE POSITIVE.

2. NAMED ECONOMISTS/EVENTS/BOOKS ARE LEGITIMATE PAYOFFS. Answers like "Ludwig von Mises", "Friedrich Hayek", "Frederic Bastiat", "The Bitcoin Standard", "August 15 1971 (Nixon shock)", "Jekyll Island 1910", "the 1920 calculation paper" are ON-VOICE, not "boring dates" or "dead names".

3. CATEGORY-MATCH is expected in Pillar 6 (reasoning-move recognition) ladders BY DESIGN. When a stem asks "which reasoning move does the critic press?" and all four choices are reasoning moves, that IS parity, not category-match failure. Only flag category-match when ONE choice fits a stated category and the OTHER THREE OBVIOUSLY DO NOT.

4. RECOGNITION-NOT-VERDICT is the pedagogical rail (feedback-surface-good-critique). Stems present a policy scenario; the answer names the CRITIC'S move, not the policy verdict. That structure is CORRECT and should NOT be flagged as "restatement" merely because the stem describes the policy the critic critiques.

5. Only flag RESTATEMENT when the answer LITERALLY echoes a DISCRIMINATING clause from the stem (e.g., stem says "he wrote the 1920 calculation paper" and answer is "the 1920 calculation paper"). If the stem sets a SCENE (a senator proposes X) and the answer names a MOVE (Bastiat's seen-vs-unseen), that is CORRECT bank-of-cards teaching, not restatement.

STILL FLAG when genuine problems exist: fabricated stats, wrong dates, wrong attributions, actual generic labels ("capitalism", "the government", "the economy", "regulation" as answers -- these ARE dead), stems that literally spell out the answer word, dangling-pronoun stems.

For EACH rung: verdict 'keep' or 'flag'; if flag: severity (high/medium/low) + rules_flagged + primary_flaw + fix. Be the adversary the first judge wasn't -- BUT do not create false positives against the Bastiat Pattern voice. ladder_ok = true ONLY if nothing is HIGH or MEDIUM.
...
def prompt_coordinator(idxs):
    ...
    _batch_tag = f"{idxs[0]:04d}-{idxs[-1]:04d}"
    out = os.path.join(STATE_DIR, f"coordinator_batch_{_batch_tag}.md")
    body = f"""{rules_prefix()}
...
def finalize(idxs):
    ...
    # Auto-integrate against the economics subject so we never accidentally point
