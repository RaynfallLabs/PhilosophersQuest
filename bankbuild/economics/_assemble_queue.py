"""Assemble the per-slice spec files into register.json + _queue.json for the economics bank v2.

Reads bankbuild/economics/_strands/spec_*.json (each a JSON array of topic-ladder specs), adds a unique
...
deep>standard>mini), groups by `section` (austrian_foundations|bastiat_seen_unseen|sound_money_bitcoin|
fed_central_banking|communism_planning_failure|public_choice_reasoning_moves), and writes both files
with coverage stats. Re-runnable.

Usage: python bankbuild/economics/_assemble_queue.py
"""
...
SD    = os.path.join(ROOT, "bankbuild", "economics", "_strands")
QUEUE = os.path.join(ROOT, "bankbuild", "economics", "_queue.json")
REG   = os.path.join(ROOT, "bankbuild", "economics", "register.json")
...
SORDER = {"austrian_foundations": 0, "bastiat_seen_unseen": 1, "sound_money_bitcoin": 2,
          "fed_central_banking": 3, "communism_planning_failure": 4, "public_choice_reasoning_moves": 5}
SECTION_NAME = {
    "austrian_foundations":          "Austrian foundations (Menger, Bohm-Bawerk, Mises, Hayek, Rothbard, Kirzner + modern)",
    "bastiat_seen_unseen":           "Bastiat + seen/unseen tradition (Bastiat's classics + seen/unseen applied to modern policy)",
    "sound_money_bitcoin":           "Sound money + Bitcoin (gold/fiat history, Bitcoin engineering, cypherpunks + adoption)",
    "fed_central_banking":           "Federal Reserve + central-banking capture (founding through Volcker + modern QE era)",
    "communism_planning_failure":    "Communism + planning failure (death record + calculation debate + price controls)",
    "public_choice_reasoning_moves": "Public choice + reasoning-move recognition (Buchanan, Sowell, Friedman + policy applications)",
}
...
reg = {"subject": "economics", "total_topics": len(topics),
