...
- **TIER 4 — NUMBERS**: weak — use ONLY when SINGULAR and UNFORGETTABLE **AND the question CONSTRUCTS the number as the payoff**: the stem must build an expectation the number *shatters* (Everett's 13,000-word oration → Lincoln's 272). A famous number asked **cold** — nothing in the stem sets it up, or the real wonder was already stated and the number is a leftover — is **randomness, not wonder → flag it**. The blessed examples (Wright 12 seconds, Lincoln 272 words, Joan at 19, Mandela 27 years, Halley's 76 years) *assume* that construction; a number on the list is **not a hall pass**. **Never as a magnitude-pick** (`"about how many millions"`) and **never at a low tier**. *(We bless wonder, not randomness — the difference is whether the question is engineered to make the number land.)*
