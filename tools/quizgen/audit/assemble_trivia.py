"""Assemble the trivia bank from `_trivia_gen/A*.json` outputs.

Reads every `A*.json` / `R*.json` file from `_trivia_gen/`, validates
each question against the full trivia gate suite, dedups within the new
bank, strips agent metadata, archives the legacy bank, and writes the
new bank to `data/questions/trivia.json`.
...
Usage:
  py -m tools.quizgen.audit.assemble_trivia [--dry-run]
"""
...
# === EXPLICIT trivia constants — verify before run ===
SUBJECT_NAME = "trivia"
BANK_PATH = REPO / "data" / "questions" / "trivia.json"
DROPPED_PATH = REPO / "data" / "questions" / "dropped" / "trivia.json"
ASSEMBLY_LOG_PATH = REPO / "_trivia_assembly_log.md"
SOURCE_DIR = REPO / "_trivia_gen"
SOURCE_GLOB_PATTERNS = ["A*.json", "R*.json"]
DROP_REASON = "v2_rebuild_2026_05_27_trivia_from_scratch_easter_egg_pattern"
...
def assemble(dry_run: bool = False) -> dict:
    ...
    assert SUBJECT_NAME == "trivia", "SUBJECT_NAME mismatch — refuse to run"
    assert BANK_PATH.name == "trivia.json", f"BANK_PATH wrong: {BANK_PATH.name}"
    assert DROPPED_PATH.name == "trivia.json", f"DROPPED_PATH wrong: {DROPPED_PATH.name}"
