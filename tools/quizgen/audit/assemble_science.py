"""Assemble the science bank from `_gen_science_*.json` outputs.

Reads every `_gen_science_*.json` at repo root, validates each question against
the full science gate suite, dedups within the new bank, strips agent metadata,
...
Aggregator priority (when multiple sources have the same tier/pillar):
  1. Tier-complete files first (`_gen_science_t1.json` through `t5.json`)
  2. Topic-supplement files for gaps (`_gen_science_t<N>_p<P>.json`)
...
Usage:
  py -m tools.quizgen.audit.assemble_science [--dry-run]
"""
...
def load_sources() -> dict[str, list[dict]]:
    """Returns {filename: [questions]} for every _gen_science_*.json."""
    ...
    tier_files = sorted(REPO.glob("_gen_science_t?.json"))
    topic_files = sorted(REPO.glob("_gen_science_t?_p*.json"))
...
def assemble(dry_run: bool = False) -> dict:
    ...
    # Validate every accepted question against the full science gate suite.
    print()
    ...
    for i, q in enumerate(accepted):
        r = validate_rewrite(
            "science", q, bank=accepted, dup_index=dup, answer_index=ans, replace_idx=i
        )
        if r["verdict"] == "FAIL":
            fail_count += 1
    ...
    dropped_path = REPO / "data" / "questions" / "dropped" / "science.json"
    if bank_path.exists():
        ...
        for q in legacy:
            qcopy = dict(q)
            qcopy["_drop_reason"] = "v2_rebuild_2026_05_26_science_from_scratch"
        ...
        print(f"Archived {len(legacy)} legacy questions to dropped/science.json "
    ...
    # Write assembly log
    log_path = REPO / "_science_assembly_log.md"
    out: list[str] = []
    out.append("# science bank assembly log")
    out.append("")
